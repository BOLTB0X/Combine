...
- Combine에서는 *Publisher* 가 data를 계속해서 발행하고 *Subscriber* 가 그 data를 구독하는 구조이므류 `Cancellable` 은 이러한 구독을 사용자가 원할 때 중단 시키는 것
...
- `AnyCancellable` 은 이를 실제로 구현한 타입으로, 주로 Combine에서 구독을 취소할 때 사용

- **구독 관리**

  `AnyCancellable` 을 통해 구독이 자동으로 관리되며, 더 이상 필요하지 않을 때 구독을 취소 가능

- **자동 구독 취소**

  `AnyCancellable` 객체가 메모리에서 해제될 때(`deinit`될 때), 구독이 자동으로 취소가 됨
...
- **Type Erasure**
  ...
  `AnyCancellable`은 타입 소거(type erasure) 방식으로 여러 종류의 `Cancellable`을 하나의 타입으로 다룰 수 있게 함
